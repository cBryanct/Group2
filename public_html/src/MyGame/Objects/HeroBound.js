...
function HeroBound(spriteTexture, atX, atY){
    ...
    this.mBoundBox.getXform().setSize(2.5, 2.5);
...
HeroBound.prototype.update = function(BoundedHero,StoneSet){
    ...
    for (var i=0; i<StoneSet.size(); i++) {
        ...
        if (this.pixelTouches(obj, p)) {
            if(BoundedHero.shieldCanDraw===true){
                BoundedHero.shieldCanDraw=false;
            }else{
                BoundedHero.decreaseHP(5);
            }
            StoneSet.removeFromSet(obj);
        }
    }
};